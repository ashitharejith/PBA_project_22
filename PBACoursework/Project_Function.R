...
#
# ************************************************
myEvaluateClassifier<-function(probs,testing_data,threshold,plot_conf=FALSE) {
  
  predictedClass<-ifelse(as.numeric(probs)<threshold,0,1)
  expectedClass<-testing_data[,OUTPUT_FIELD]
  
  results<-TcalcConfusion(expectedClass=expectedClass,
...
myPerformancePlot<-function(probs,testing_data){
  ...
  plot(x=toPlot$x,
       ...
       xlab="Threshold",
       ylab="%Rate",
       main="Threshold Perfomance Rain Prediction Model")
  ...
  legend("topright",c("TPR","1-FPR","Distance","Youdan"),col=c("blue","red","green","purple"),lty=1:2,lwd=2,cex=0.75)
  text(x=0,y=50, adj = c(-0.2,2),cex=1,col="black",paste("THRESHOLDS:\nEuclidean=",minEuclidean,"\nYoudan=",maxYoudan))
  
  # ************************************************
  # 121020NRT ROC graph
  
  plot(100-toPlot$fpr,toPlot$tpr,type="l",lwd=3, col="blue",main="ROC Rain Prediction Model",
  ...
  results$AUC<-auroc(score=probs,bool=testing_data)
 #results$AUC<-auroc(score=test_predicted,bool=test_expected)
...
#
# ************************************************
NcalcConfusion<-function(expectedClass,predictedClass,plot_conf=FALSE){
